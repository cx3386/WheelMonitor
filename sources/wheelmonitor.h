...
class PLCSerial;
class QThread;
class WheelMonitor : public QMainWindow
{
	Q_OBJECT
public:
	WheelMonitor(QWidget *parent = Q_NULLPTR);
	~WheelMonitor();
...
protected:
	...
	void execPlanningTasks();
	void setupPlanningTasks(int hh, int mm, bool execnow = true, bool everyday = true);
