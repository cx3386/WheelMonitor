...
ImageProcess::ImageProcess(FaultPlayBackWidget &w, QObject *parent)
	: QObject(parent)
	, nImgCount(0)
	, nFragments(0)
	, bStopProcess(true)
...
void ImageProcess::doImageProcess()
{
	...
	rectangle(frameToShow, g_imgParam.roiRect, Scalar(0, 0, 255), 2); //BGR
	mutex.unlock();
	if (!bStopProcess)
	{
		//when wheel is stop,
...
int ImageProcess::coreImageProcess() //0-no wheel, 1-matches success, 2-wait next srcImg
{
	...
	equalizeHist(roiImage, roiImage); //equalize the image //2017/12/21
	...
	Mat maskTmp = rMatcher.getMask(imageRing.size(), radiusOutside - 10, radiusInside + 10); //get a ring mask, use to match
...
void ImageProcess::alarmThisWheel()
{
	...
	int alarmLevel = 0;
	/*ocr*/
	qDebug() << "ImageProcess: ocr result size" << ocr->get_results_size();
	...
	else
	{
		...
		if (validMatchCount >= 10)
		{
			if (fabs(meanDiff) > meanRef * g_imgParam.warningRatio)
			{
				...
				else if (faultPlayBackWidget.isLastAlarm(num)) //if the last wheel is alarmed
				{
					emit setAlarmLight(PLCSerial::AlarmColorRed);
	...
	for (auto&& sp : rtSpeeds) {
		speedsStr += QString(" %1").arg(sp);
	}
	speedsStr += ";";
	for (auto&& sp : refSpeeds) {
		speedsStr += QString(" %1").arg(sp);
	}
	faultPlayBackWidget.insertRecord(num, nFragments, totalMatchCount, validMatchCount, alarmLevel, meanDiff, meanRef, speedsStr, HikVideoCapture::videoRelativeFilePath);
