...
1. 安装完成后，安装虚拟串口软件（如果此前没有安装的话）。方法：打开安装目录(默认为C:/Program Files/ZJU/WheelMonitor)下的tools文件夹，运行ZLVircom4.96_x64.msi。
1. 将监测系统的网线连接至电脑，将电脑IP配置到同一网段（192.168.1.*），并测试是否能够ping通摄像机
1. 打开虚拟串口软件，配置虚拟串口
1. 打开监测软件，开始监测

- 注意：**K-Lite解码包**和**ZLVircom**是**商业软件**。
- 如果安装过程中提示系统缺少Microsoft Visual C++ 2017 Redistrubutable，请联网后重新安装。或者手动安装vc_redist.x64.exe

## 卸载

请到控制面板中卸载程序。注意：ZLVirCom需要手动卸载
